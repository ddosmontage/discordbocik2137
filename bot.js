...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Status serwera", "https://i.imgur.com/vaMwhGe.jpg")
    .setColor("#F1F8E9")
    .setFooter('Ostatnia aktualizacja:')
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('Status serwera:','Offline',true)
    .addField('Ilość graczy online:','0/64\n\u200b\n',true)
    .setFooter('GhostTown');
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Status serwera:','Online',true)
        .addField(' ­',' ­',true)
        .addField('Liczba graczy:',`${players.length}/${64}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('Status Serwera Fivem');
